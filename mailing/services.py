    ...
    for client in mailing.clients.all():
        try:
            ...
            MailingAttempt.objects.create(
                mailing=mailing, status="ok", server_response="Отправлено успешно", timestamp=timezone.now()
            )

            logger.info(f"Успешно отправлено письмо клиенту {client.email} для рассылки ID={mailing.pk}")

            results.append({"client": client, "status": "ok", "response": "Отправлено успешно"})

        except Exception as e:
            MailingAttempt.objects.create(
                mailing=mailing, status="fail", server_response=str(e), timestamp=timezone.now()
            )

            logger.error(f"Ошибка при отправке письма клиенту {client.email} (рассылка ID={mailing.pk}): {e}")

            results.append({"client": client, "status": "fail", "response": str(e)})
